chrome.tabs.getCurrent(function(tab) {
  let isBlankTab = (tab.url.indexOf('newtab') !== -1) ? true : false;
});

window.onload = function(isBlankTab){
if(isBlankTab){
  (function() {
      let screen_w = Math.max(document.documentElement.clientWidth, window.innerWidth || 0),
         screen_h = Math.max(document.documentElement.clientHeight, window.innerHeight || 0);
      debug = false;
      let destroy_list = [];
      let searchTerms = [];

      let bgcolor_pref = (localStorage['albumized_pref_bgcolor'] == undefined) ? '#101019' : '#'+localStorage['albumized_pref_bgcolor'],
      ...
      user_data = 1;
      let album_one = document.getElementById('album1'),
      ...
      let SCALE,canvas,world,fixDef,ctx,width,height,shapes = {},needToDraw = false;easterEggEnabled = false;
      let albumLink0,albumLink1,albumLink2,albumLink3,albumLink4,albumLink5,albumLink6,albumLink7,
         albumLink8,albumLink9,albumLink10,albumLink11,albumLink12,albumLink13,albumLink14,albumLink15,
         albumLink16 = 'https://witness6.bandcamp.com/album/lifecycles?referrer=Albumized!';
      ...
      let init = {
        ...
        start: function(id) {
          ...
          this.displayPrefsLink();
            ...
            let canv = document.getElementById('canvas-body');
            canv.style.transition = 'background 0.2s ease';
            ...
            easter_egg.load();
            bdy.style.transition = 'background 0.2s ease';
        ...
        displayPrefsLink: function() {
            let prefzlink = chrome.runtime.getURL('../options.html');
            $('a').click(function(){window.location = prefzlink});
        },

        getLocalizedLink: function(url) {
            let userLocale = chrome.i18n.getUILanguage() || 'en-US';
            ...
            url = url.replace(url.split('/')[3], iTunesCountry) + '&itscg=30200&itsct=music_box_link&ls=1&app=music&mttnsubad=1667990774&at=11l6841';
            return url;
        },
        ...
        itunesLookup: function(specialday) {
            ...
            if (specialday === 16) {
              ...
              $.ajax({
                url: 'https://raw.githubusercontent.com/rottina/albumized/main/lib/8fac666a0a596d41fe05ef3d83eca175.json',
                dataType: 'json',
                async: false,
            ...
            let artist, song, albumCover, albumUrl, regex = /100x100/;
        ...
        getDate: function() {
            let d = new Date(), mm = d.getMonth() + 1, day = d.getDate(), holidaydigits;
            if (mm == 1 && day == 1) {
                holidaydigits = 2;
            } else if (mm == 2 && day == 14) {
                holidaydigits = 16;
      ...
      let box2d = {
          addToWorld: function(shape) {
              let bodyDef = this.create.bodyDef(shape);
              let body = this.create.body(bodyDef);
              if (shape.radius) {
      ...
      //mouse
       ...
       let canvasPosition = getElementPosition(document.getElementById('canvas-body'));
       let canvasID = document.getElementById('canvas-body');
